...
class _AppShellState extends ConsumerState<AppShell>
  ...
  /// One of the 4 real shell-branch tabs, keyed by its actual index in
  /// [_tabs] regardless of where it's drawn in the Row — used for both the
  /// tabs before the optional 5th slot and (with index == _tabs.length - 1)
  /// the trailing Profile tab drawn after it.
  Widget _buildTabButton({
    required int index,
    required int selectedIndex,
    required StatefulNavigationShell navigationShell,
    required AuthGateController authGate,
    required AppRouteLoadingNotifier routeLoadingController,
          ...
          child: DecoratedBox(
            ...
            child: SafeArea(
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    // The optional 5th slot renders second-to-last — right
                    // before Profile, not appended after it — so Profile
                    // always stays the final, rightmost tab. Built from the
                    // real tab list (last index held back, then appended
                    // after the extra slot) rather than a hardcoded split so
                    // this still holds if _tabs ever gains/loses entries.
                    for (int index = 0; index < _tabs.length - 1; index++)
                      _buildTabButton(
                        index: index,
                        selectedIndex: selectedIndex,
