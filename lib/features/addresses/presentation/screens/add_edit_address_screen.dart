...
class _AddEditAddressScreenState extends ConsumerState<AddEditAddressScreen> {
  ...
  void _seedFromInitialAddress() {
    ...
    // Deliberately NOT address.name here — unlike receiverName (the real,
    // nullable per-address value) and accountName (the real account name),
    // AddressEntity.name is a required, always-non-null DISPLAY fallback
    // (see AddressModel.toEntity / address_provider.dart's fallbackName) —
    // literally the string "Bakaloo Customer" for an account with no name
    // yet. Including it here fed that placeholder into these fields as if
    // it were real data, so a first-time user completing their GPS-detected
    // address (forceCompletion, initialAddress != null) saw First/Last Name
    // pre-filled with "Bakaloo"/"Customer" instead of blank — and saving
    // without noticing then permanently set the account's real name to
    // that placeholder via the auto-save-as-account-name logic below.
