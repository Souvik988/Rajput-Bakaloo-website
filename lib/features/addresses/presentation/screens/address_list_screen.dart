...
final _addressListCurrentPositionProvider = FutureProvider<Position?>((
  ...
  try {
    ...
    // Same tiered cached/low/medium-accuracy fallback used by every other
    // location call site in the app (see resilient_location.dart) — a bare
    // getCurrentPosition() here had no timeLimit at all, so on a cold GPS
    // fix (e.g. right after the app starts) it could sit pending
    // indefinitely instead of ever resolving to a distance label.
    return await getResilientCurrentPosition();
  } catch (_) {
    return null;
  }
});
