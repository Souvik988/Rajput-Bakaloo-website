...
class PriceModeNotifier extends _$PriceModeNotifier {
  ...
  Future<void> _clearPriceSensitiveState() async {
    ...
    // The 5th bottom-nav slot and the Profile screen's PROFILE_MENU list
    // are server-resolved by audience (B2C/B2B) the same way banners and
    // section manifests are — without invalidating these, a button
    // configured as B2C-only (e.g. "Payment") keeps showing after
    // switching to wholesale, and a B2B-only counterpart (e.g. "Payment
    // B2b") never appears, until something unrelated refetches them.
    try {
      ref
        ..invalidate(navButtonProvider)
        ..invalidate(navButtonProfileMenuProvider);
    } catch (_) {}
