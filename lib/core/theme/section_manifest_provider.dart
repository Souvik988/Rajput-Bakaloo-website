...
Future<void> refreshSectionManifest(WidgetRef ref, String tabKey) async {
  ...
  final List<String> keysToRemove = _memoryCache.keys
      .where(
        (String key) =>
            key.split('::').length >= 2 && key.split('::')[1] == tabKey,
      )
...
// The audience (B2C vs B2B) a section manifest resolves to is derived
// server-side purely from the logged-in account's own B2B status — not
// from a client-side toggle — so the only reliable way to keep this
// device's cache from bleeding one account's content into another is to
// key it by account too. Same `HiveService.userBox.get('user')` read
// remote_theme_provider.dart's own `_currentUserId()` uses for the
// identical reason (see that file for the sibling cache). Without this,
// a device that has ever loaded a tab while logged into a B2B account
// keeps serving that content after switching to a B2C account (or vice
// versa) — the actual cause of a correctly-authored, live "All" tab
// appearing empty only on a device that had previously seen it as B2B.
...
String _cacheKey(String storeKey, String tabKey) =>
    '$storeKey::$tabKey::${_currentUserId() ?? 'anon'}';

String _manifestCacheKey(String storeKey, String tabKey) =>
    'section_manifest_${storeKey}_${tabKey}_${_currentUserId() ?? 'anon'}';
