...
1. **Device GPS on the test machine is OS-blocked — environment, not app.** Windows' location API denies every client (`WinRT RequestAccessAsync → Denied`, `GetGeopositionAsync → 0x80070005`) while `lfsvc` runs and user-level consent is Allow — the signature of the device-level **"Location services" toggle being OFF** (Settings → Privacy & security → Location). It flipped off since the user's last successful test. The app's failure path is graceful (existing manual-address flow); flipping the toggle restores GPS immediately. The user declined both UAC elevation attempts, so the toggle remains user-action.
