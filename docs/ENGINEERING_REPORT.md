...
1. **Device GPS on the test machine — FIXED end-to-end.** Root-cause chain (all verified in the browser): (a) Windows had four permission gates denying every client — device "Location services" toggle off (`lfsvc Status 0→1`), machine desktop-app consent empty→Allow, **user-level master consent set to `Deny`** (the flip that broke it) →Allow, and missing per-app entries for the WebView2/host processes →added; the consent broker needed a reload before positions flowed again. (b) The app's web location path had three intermittent failure modes, all fixed: cold browser geolocation provider could exceed the first timeout → `resilient_location.dart` now retries the live fix across accuracy/timeout tiers on web (mobile tiered sequence unchanged); geolocator_web mapped transient errors inside `requestPermission` to `deniedForever` → `resolveLocationPermission` re-queries the browser state on web and downgrades false permanents to retryable `denied`; a transient `getStyle` failure was cached as "map unavailable" for the whole picker visit → `OlaMapsService.getStyle` retries exceptions (genuine `configured:false` still returns immediately, and the picker keeps its Retry button). Verified across repeated fresh page loads: Add Address auto-detects the real GPS position, opens the Ola map centered there ("Pin location is 0.0 kms away"), Confirm fills Address/Landmark/City/PIN/State via Ola reverse geocode, serviceability shows "Delivery available in 59 mins" — no error banner, reproducible.
2. **Ola Maps on localhost — fixed for dev via proxy.** The backend builds `styleUrl` from its own public host and only emits CORS headers for `*.bakaloo.in` origins, so from localhost the MapLibre style document was CORS-blocked and the map rendered blank. The dev proxy (`tool/dev_cors_proxy.js`) now rewrites `styleUrl` to itself so the style loads same-origin; Ola's tile/sprite/glyph CDN already sends `Access-Control-Allow-Origin: *`. **Production origins need no fix** — verified the map picker (tiles, live Ola search suggestions, select→camera move, confirm→reverse geocode→serviceability gate) and the order-tracking screen (Ola route polyline, rider placeholders) fully working in the browser.
3. **Cart retention after a scheduled (COD) order**: the backend keeps cart lines after placing a scheduled order (they cleared for the online-verify path only). Backend-owned behavior; identical to mobile against this backend.
4. **Localhost CORS for the REST API**: the production API allowlists `*://*.bakaloo.in` (verified). Localhost dev origins are not allowlisted — local testing uses `tool/dev_cors_proxy.js` (dev harness only). **No backend change needed for deployment** on any bakaloo.in origin.
5. **Push notifications (FCM) and in-app review prompts are absent on web** (no browser integration in the baseline's Firebase setup); in-app notification history works. Razorpay runs with the backend's configured key — verified in **Test Mode**; production keys activate automatically server-side.
6. **Wasm**: the build targets JS (default). A `--wasm` build would be blocked by transitive packages still using `dart:html` (`flutter_secure_storage_web`, `location_web`) — a Flutter-ecosystem limitation, unrelated to this port.
7. **Deep URLs use hash strategy** (`/#/product/:id`) — Flutter web's default; refresh-safe on any static host. Path-style URLs would require host rewrites and were not introduced.
8. **Access tokens expire every ~15 minutes**; the refresh interceptor renews them (verified: `/auth/refresh-token → 200` then requests succeed). A screen opened across an expiry boundary may need one retry — the source's designed behavior (unit-tested).
9. **Deployment artifact**: `build/web-production` — built with the production env (verified: `assets/.env` = `https://api.bakaloo.in/api/v1`). Code-identical to the browser-verified test bundle; only the env asset differs. Rebuild any time with `cp .env.example .env && flutter build web --release -o build/web-production`. `build/web` remains the local test bundle served by `tool/dev_servers.js`.
10. **"Service unavailable" blocker seen during local testing — dev-harness cause, not an app defect.** The app's availability gate (5 consecutive network-level failures) escalated because the local CORS proxy process kept dying, so API calls failed with connection errors. Fixed for local testing with `tool/dev_servers.js` — one self-recovering process hosting both the static server (8096) and the proxy (9443). In production the app talks to `api.bakaloo.in` directly (Cloudflare-fronted); no local process is involved.
