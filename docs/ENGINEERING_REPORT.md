...
9. `build/web` **currently contains the local test bundle** (`.env` pointing at the local proxy, used for interactive testing). For deployment: restore the production env (`cp .env.example .env`) and rebuild — `flutter build web --release`. The production-env bundle was built and verified earlier.
