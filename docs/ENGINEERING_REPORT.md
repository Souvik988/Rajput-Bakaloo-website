...
1. **Device GPS on the test machine is OS-blocked — environment, not app.** Full diagnosis chain: browser geolocation fails instantly with `POSITION_UNAVAILABLE`; Windows' own API (`WinRT RequestAccessAsync`) returns `Access denied (0x80070005)` even with `lfsvc` running. Root causes found and fixed in the registry (all now verified **Allow**): device "Location services" master toggle (`lfsvc Status` 0→1, elevated), machine "desktop apps" consent (`NonPackaged` empty→Allow, elevated), and — the one that flipped since the user's last successful test — the **user-level master consent was `Deny`** (HKCU root, fixed without elevation), plus per-app entries for the embedded WebView2/host processes. The consent broker caches decisions in memory, so **a reboot (or flipping the toggles in Settings → Privacy & security → Location) is required** for the fixes to take effect — the sanctioned UI path is the ground truth on Windows 11. Meanwhile the app degrades gracefully: the manual address form and the Ola map picker (search + tap-to-pick + reverse geocode) work fully without GPS, and production end users are unaffected (their own devices control their own permissions).
